...
import Jisp.lang.Abstract.AFn;
import Jisp.lang.Abstract.APersistentMap;
...
//                                                  Symbol.intern("SortedSet"), "java.util.SortedSet"
    ...
    //boolean
    static final public Var UNCHECKED_MATH = Var.intern(Namespace.findOrCreate(Symbol.intern("clojure.core")), Symbol.intern("*unchecked-math*"), Boolean.FALSE).setDynamic();
...
//                                                    Module.findOrCreateModule("clojure/user"));
    ...
    static public void addURL(Object url) throws MalformedURLException {
        ...
        else throw new IllegalAccessError("Context classloader is not a DynamicClassLoader");
    }
    ...
    static {
        ...
        v = Var.intern(CLOJURE_NS, LOAD_FILE, new AFn() {
            public Object invoke(Object arg1) {
                try {
                    return java.lang.Compiler.loadFile((String) arg1);
                } catch (IOException e) {
    ...
    public static void loadResourceScript(Class c, String name, boolean failIfNotFound) throws IOException {
        ...
        if (ins != null) {
            try {
                java.lang.Compiler.load(new InputStreamReader(ins, UTF8), name, file);
            } finally {
                ins.close();
            }
    ...
    static void compile(String cljfile) throws IOException {
        ...
        if (ins != null) {
            try {
                java.lang.Compiler.compile(new InputStreamReader(ins, UTF8), cljfile, cljfile.substring(1 + cljfile.lastIndexOf("/")));
            } finally {
                ins.close();
            }
    ...
    static public void load(String scriptbase, boolean failIfNotFound) throws IOException, ClassNotFoundException {
        ...
        if (!loaded && cljURL != null) {
            if (booleanCast(java.lang.Compiler.COMPILE_FILES.deref())) compile(scriptfile);
            else loadResourceScript(RT.class, scriptfile);
        } else if (!loaded && failIfNotFound)
            throw new FileNotFoundException(String.format("Could not locate %s, %s or %s on classpath.%s", classfile, cljfile, cljcfile, scriptbase.contains("_") ? " Please check that namespaces with dashes use underscores in the Clojure file name." : ""));
    }
...
////////////// Collections support /////////////////////////////////
        ...
        if (iter.hasNext()) {
            return new LazySeq(new AFn() {
                public Object invoke() {
                    ...
                    int n = 0;
                    while (iter.hasNext() && n < CHUNK_SIZE) arr[n++] = iter.next();
    ...
    static ISeq seqFrom(Object coll) {
        ...
        else if (coll instanceof Iterable) return chunkIteratorSeq(((Iterable) coll).iterator());
    ...
    static public boolean canSeq(Object coll) {
        return coll instanceof ISeq || coll instanceof Seqable || coll == null || coll instanceof Iterable || coll.getClass().isArray() || coll instanceof CharSequence || coll instanceof Map;
    }

    static public Iterator iter(Object coll) {
        if (coll instanceof Iterable) return ((Iterable) coll).iterator();
        else if (coll == null) return new Iterator() {
    ...
    static public ISeq keys(Object coll) {
        if (coll instanceof IPersistentMap) return APersistentMap.KeySeq.createFromMap((IPersistentMap) coll);
        else return APersistentMap.KeySeq.create(seq(coll));
    }

    static public ISeq vals(Object coll) {
        if (coll instanceof IPersistentMap) return APersistentMap.ValSeq.createFromMap((IPersistentMap) coll);
        else return APersistentMap.ValSeq.create(seq(coll));
    }
    ...
    static int countFrom(Object o) {
        ...
        else if (o instanceof Map.Entry) return 2;
...
//}
    ...
    static public Object contains(Object coll, Object key) {
        ...
        else if (coll instanceof IPersistentSet) return ((IPersistentSet) coll).contains(key) ? T : F;
        else if (coll instanceof Map) {
            Map m = (Map) coll;
        ...
        } else if (coll instanceof ITransientSet) return ((ITransientSet) coll).contains(key) ? T : F;
    ...
    static public Object find(Object coll, Object key) {
        if (coll == null) return null;
            ...
            Map m = (Map) coll;
            if (m.containsKey(key)) return MapEntry.create(key, m.get(key));
            return null;
        } else if (coll instanceof ITransientAssociative2) {
...
//takes a seq of key,val,key,val
    ...
    static Object nthFrom(Object coll, int n) {
        if (coll == null) return null;
        else if (coll instanceof CharSequence) return Character.valueOf(((CharSequence) coll).charAt(n));
        else if (coll.getClass().isArray())
    ...
    static Object nthFrom(Object coll, int n, Object notFound) {
        if (coll == null) return notFound;
            ...
            if (groups > 0 && n <= m.groupCount()) return m.group(n);
            return notFound;
        } else if (coll instanceof Map.Entry) {
            Map.Entry e = (Map.Entry) coll;
            if (n == 0) return e.getKey();
    ...
    static public Object assocN(int n, Object val, Object coll) {
        if (coll == null) return null;
        else if (coll instanceof IPersistentVector) return ((IPersistentVector) coll).assocN(n, val);
    ...
    static public IPersistentMap map(Object... init) {
        ...
        else if (init.length <= PersistentArrayMap.HASHTABLE_THRESHOLD) return PersistentArrayMap.createWithCheck(init);
        return PersistentHashMap.createWithCheck(init);
    }

    static public IPersistentMap mapUniqueKeys(Object... init) {
        if (init == null) return PersistentArrayMap.EMPTY;
        else if (init.length <= PersistentArrayMap.HASHTABLE_THRESHOLD) return new PersistentArrayMap(init);
    ...
    static public Object[] object_array(Object sizeOrSeq) {
        if (sizeOrSeq instanceof Number) return new Object[((Number) sizeOrSeq).intValue()];
        else {
    ...
    static public Object[] toArray(Object coll) {
        if (coll == null) return EMPTY_ARRAY;
            ...
            for (int i = 0; i < ret.length; i++, s = s.next())
                ret[i] = s.first();
            return ret;
        } else throw Util.runtimeException("Unable to convert: " + coll.getClass() + " to Object[]");
    }
...
///////////////////////////////// reader support ////////////////////////////////
    ...
    static public int getColumnNumber(Reader r) {
        if (r instanceof LineNumberingPushbackReader) return ((LineNumberingPushbackReader) r).getColumnNumber();
        return 0;
    }
    ...
    static public void print(Object x, Writer w) throws IOException {
        //call multimethod
        if (PRINT_INITIALIZED.isBound() && RT.booleanCast(PRINT_INITIALIZED.deref())) PR_ON.invoke(x, w);
//*
        else {
            ...
            if (x instanceof Obj) {
                ...
                if (RT.count(o.meta()) > 0 && ((readably && booleanCast(PRINT_META.deref())) || booleanCast(PRINT_DUP.deref()))) {
                    ...
                    w.write("#^");
                    if (meta.count() == 1 && meta.containsKey(TAG_KEY)) print(meta.valAt(TAG_KEY), w);
                    else print(meta, w);
                    w.write(' ');
                }
            }
...
//			getRootClassLoader();
    ...
    static public ClassLoader baseLoader() {
        if (java.lang.Compiler.LOADER.isBound()) return (ClassLoader) java.lang.Compiler.LOADER.deref();
        else if (booleanCast(USE_CONTEXT_CLASSLOADER.deref())) return Thread.currentThread().getContextClassLoader();
        return java.lang.Compiler.class.getClassLoader();
    }
    ...
    static public Class classForName(String name, boolean load, ClassLoader loader) {
        ...
        try {
            Class c = null;
            if (!(loader instanceof DynamicClassLoader)) c = DynamicClassLoader.findInMemoryClass(name);
            if (c != null) return c;
